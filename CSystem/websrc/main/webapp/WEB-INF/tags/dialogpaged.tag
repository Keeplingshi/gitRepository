...
<style type="text/css">
	...
	<div >
		<table class="ui-pg-table" style="margin:auto;">
			<tbody>
				<tr>
					...
					<td dir="ltr">
						Page <input id="ui-pg-input" class="ui-pg-input" type="text" size="2" maxlength="7" value="${pagedialogInfo.currentPageNo }" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)" onblur="this.v();" onkeypress="EnterPress(event)" onkeydown="EnterPress()" > of 
						<span id="sp_1_grid-pager">${pagedialogInfo.totalPages }</span>
					</td>
					<td class="ui-pg-button ui-state-disabled" style="width: 4px; cursor: default;">
						<span class="ui-separator"></span>
					</td>
		...
		<input type="hidden" id="currentPageNo" name="pagedialogInfo.currentPageNo" value="${pagedialogInfo.currentPageNo}" >
		<input type="hidden" id="sizePerPage" name="pagedialogInfo.sizePerPage" value="${pagedialogInfo.sizePerPage}" >
		<input type="hidden" id="totalPages" name="pagedialogInfo.totalPages" value="${pagedialogInfo.totalPages}" >
	</div>
	
<script>

	$(function(){
		var pageSize = "${pagedialogInfo.sizePerPage}";
	...
	$("#prev_grid-pager").click(function(){
		if("${pagedialogInfo.currentPageNo}"==1){
			layer.msg("当前为第一页！", {
				offset: ['260px'],
				time: 1500//1.5s后自动关闭
			});
		...
		$("#currentPageNo").val("${pagedialogInfo.currentPageNo-1}");
		doPage();
	});
	
	$("#next_grid-pager").click(function(){
 		if("${pagedialogInfo.currentPageNo}"=="${pagedialogInfo.totalPages}"){
			layer.msg("当前为最后一页！", {
				offset: ['260px'],
				time: 1500//1.5s后自动关闭
			});
	...
	$("#last_grid-pager").click(function(){
		$("#currentPageNo").val("${pagedialogInfo.totalPages}");
		layer.msg("最后一页！", {
			offset: ['260px'],
			time: 1500//1.5s后自动关闭
		});
	...
	function doPage()
	{
		var form = $("#dialogFormId");
    	form.ajaxSubmit(function(data){
    	 	var $target = $("#dialog_content_page");
  	   	 	$target.html(data);
		});
	}
